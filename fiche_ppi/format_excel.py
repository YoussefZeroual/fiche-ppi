...
def format_ppi_bold(df, filename, tag_color_map=None):
    ...
    with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
        # Ne PAS utiliser df.to_excel() — xlsxwriter interdit de réécrire
        # une cellule déjà écrite. On crée la feuille manuellement.
        ...
        for row_num in range(len(df)):
            ...
            # Set the pre-calculated row height with wrap format
            worksheet.set_row(excel_row, row_heights[excel_row], regular)
            ...
            for col_num, col_name in enumerate(df.columns):
                ...
                if pd.notna(cell_value):
                    ...
                    # Check if any special tags exist (case-insensitive)
                    has_tags = any(f'<{tag}>'.lower() in text.lower() for tag in tag_color_map.keys())
                    
                    if has_tags:
                        # Normalise les tags en majuscules pour matcher tag_color_map
                        import re as _re
                        def _upper_tags(m): return m.group(0).upper()
                        text_norm = _re.sub(r'</?[a-zA-Z]+>', _upper_tags, text)
                        parts = re.split(pattern, text_norm)
                        ...
                        if rich_string:
                            ...
                            if len(rich_string) > 0:
                                try:
                                    worksheet.write_rich_string(excel_row, col_num, *rich_string, regular)
                                except Exception:
                                    clean_text = re.sub(r'<[^>]+>', '', text)
                                    worksheet.write_string(excel_row, col_num, clean_text, regular)
                        else:
                            worksheet.write(excel_row, col_num, text, regular)
                    else:
                        worksheet.write_string(excel_row, col_num, text, regular)
                else:
                    worksheet.write_string(excel_row, col_num, '', regular)

KNOWN_TAGS = {'PPI', 'MD', 'EXP', 'APP', 'POR', 'MOD', 'BOLD'}
